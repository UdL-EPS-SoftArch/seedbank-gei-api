Feature: Update Take
  In order to update Take
  I want to update a new Take

  Background:
    Given There is a registered user with username "username" and password "password" and email "user@sample.app"
  Scenario: Update a Take with user
    Given I login as "username" with password "password"
    And I create a new Take with amount 5, weight 5 and location "Lleida"
    When I update Take with changing amount to 24
    Then The response code is 200
    And I check if the new amount is not 5

  Scenario: Update a Take with user not logged in
    Given I'm not logged in
    And There is a Take created with amount 5, weight 5 and location "Lleida"
    When I update Take with changing amount to 24
    Then The response code is 401

  Scenario: Update an inexistent Take with user
    Given I login as "username" with password "password"
    When I update Take with id 1000 changing amount to 24
    Then The response code is 404

  Scenario: Update a Take with user not logged in
    Given I'm not logged in
    When I update Take with id 1000 changing amount to 24
    Then The response code is 401